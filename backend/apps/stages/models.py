...
class Stage(models.Model):
    ...
    def missing_document_types(self):
        ...
        uploaded_docs = set(
            Document.objects.filter(
                stage=self,
                doc_type__in=self.required_document_types) .values_list(
                'doc_type',
                flat=True))
        missing = [
            doc_type for doc_type in self.required_document_types if doc_type not in uploaded_docs]
    ...
    def approval_blockers(self):
        ...
        if self.requires_minutes and hasattr(
                self, 'minutes') and not self.minutes.is_approved:
            blockers['minutes'] = [
                'Minutes of Presentation must be approved by the assigned supervisor']

        if self.requires_minutes and not hasattr(self, 'minutes'):
            blockers['minutes'] = [
                'Minutes of Presentation must be uploaded and approved']
    ...
    def check_thesis_submission_complete(self):
        ...
        uploaded_docs = set(
            Document.objects.filter(
                stage=self,
                doc_type__in=required_doc_types) .values_list(
                'doc_type',
                flat=True))

        if required_doc_types.issubset(
                uploaded_docs) and self.status != 'IN_PROGRESS':
