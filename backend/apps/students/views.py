...
class StudentViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if request.user.role_key not in ['coordinator', 'dean', 'cod', 'director_bps']:
            raise PermissionDenied('Only coordinators and senior administrators can view all students.')
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if user.role_key == 'supervisor' and student.assigned_supervisor_id == user.id:
            return super().retrieve(request, *args, **kwargs)
        raise PermissionDenied('You are not allowed to view this student record.')
