...
class ActivityViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if stage.status != 'ACTIVE':
                return Response(
                    {'error': 'Can only add activities to active stages'}, status=status.HTTP_400_BAD_REQUEST)
            if request.user.role_key == 'student' and stage.student.user != request.user:
                raise PermissionDenied(
                    'You can only add activities to your own stage.')
            if request.user.role_key == 'supervisor' and stage.student.assigned_supervisor != request.user:
                raise PermissionDenied(
                    'You can only add activities to students assigned to you.')
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if activity.stage.status != 'ACTIVE':
            return Response({'error': 'Completed stages are read-only.'},
    ...
    def mark_done(self, request, pk=None):
        ...
        if request.user.role_key == 'supervisor' and activity.stage.student.assigned_supervisor != request.user:
            raise PermissionDenied(
                'Only the assigned supervisor can mark this activity complete.')
        if request.user.role_key == 'student' and activity.stage.student.user != request.user:
            raise PermissionDenied(
                'You can only mark your own activities complete.')

        if activity.status == 'COMPLETED':
            return Response({'error': 'Activity already completed'},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def calendar(self, request):
        ...
        stage_id = request.query_params.get(
            'stage_id') or request.query_params.get('stage')
        if stage_id:
